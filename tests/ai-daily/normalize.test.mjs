...
test('normalizeSearchResults dedupes urls and keeps compact fields for enrichment', () => {
  const results = [
    {
  ...
  assert.equal(items[0].titleZh, 'OpenAI new agent tools');
  assert.equal(items[0].tags[0], 'OpenAI');
  assert.equal(items[0].snippet.length, 120);
